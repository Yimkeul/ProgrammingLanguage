let rec interp_expr (ast : Ast.expr) ( (e , m): (Env.t *  Memory.t)) : Value.t = 
  ...
  | Ast.Id x -> let addr = Env.find x e in Memory.find addr m
...
match ast with
  ...
  | Ast.LoadStmt (s1, e1) ->
    ...
    let m' = Memory.add x addr' m in (e,m')

  (* e? s1^- : s2^- *)
  (* | Ast.IfStmt  (e1,slst1, slst2) ->  
    let v = interp_expr e1 (e, m) in
      let (e', m') =
      match v with
  ...
  (* | Ast.LoopStmt (e1, slst) ->
    let rec loop (e, m) =
      ...
      match v with
      | Value.BoolV true ->
        let (e', m') = interp_stmt_list slst (e, m) in
        loop (e', m')
  ...
  | Ast.LoopStmt (e1, slst) ->
    let rec loop (e, m) =
      ...
      | _ -> failwith (Format.asprintf "[Error] Not a boolean: %a" Ast.pp_expr e1)
      in loop (e, m) 
  (* and interp_stmt_list (slst : Ast.stmt list) ((e, m) : (Env.t * Memory.t)) : (Env.t * Memory.t) =
  match slst with
  | [] -> (e, m)
  | stmt :: rest ->
    let (e', m') = interp_stmt stmt (e, m) in
...
let prog = "def x; def y; x = 3; if (x < 3) { y = 1; } else { y = 99; }"
